...
// ✅ Initialize Firebase using Render environment variables
try {
...
function enhanceWithEmojis(text) {
  ...
  t = t.replace(
    /\b(doctor|physician|medical help)\b/gi,
    "👩‍⚕️ $1"
  );
  t = t.replace(
    /\b(medicine|medication|remedy|treatment)\b/gi,
    "💊 $1"
  );
  t = t.replace(
    /\b(see a doctor|consult a doctor|seek medical)\b/gi,
    "⚕️ $1"
  );
  t = t.replace(
    /\b(diet|food|nutrition)\b/gi,
    "🥗 $1"
  );
  t = t.replace(
    /\b(exercise|walk|yoga)\b/gi,
    "🏃 $1"
  );
...
app.post("/api/process-text", async (req, res) => {
  ...
  if (!userText || !sessionId) {
    return res.status(400).json({
      error: "Missing userText or sessionId",
    });
  }

  // ✅ Quick greeting
  if (
    /^(hi|hello|hey|yo|good\s(morning|evening|afternoon))$/i.test(
      userText.trim()
    )
  ) {
    const greeting =
      "👋 Hello! I'm Manas+, your friendly health companion. How are you feeling today?";
...
4. A clear note about when to see a doctor.`,
    ...
    res.status(500).json({
      ...
      remedies: [
        "AI model not responding right now. Please try again shortly.",
      ],
    });
  }
});
...
app.listen(PORT, () => {
  console.log(
    `✅ Manas+ backend running at http://localhost:${PORT}`
  );
});
